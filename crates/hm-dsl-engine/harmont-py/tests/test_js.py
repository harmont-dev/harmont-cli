...
def test_chain_node_npm() -> None:
    ...
    assert "npm ci" in npm_ci.action.cmd

    node_install = npm_ci.parent
    assert "nodejs" in node_install.action.cmd
    assert node_install.cache is not None

    apt_base = node_install.parent
    assert "apt-get" in apt_base.action.cmd

    assert apt_base.parent.action is None  # scratch


def test_chain_node_pnpm() -> None:
    """scratch -> apt-base -> node-install -> corepack-pnpm -> pnpm-deps."""
    pnpm_deps = js.project(pm="pnpm").install()
    assert "pnpm install --frozen-lockfile" in pnpm_deps.action.cmd

    corepack = pnpm_deps.parent
    assert "corepack enable pnpm" in corepack.action.cmd

    assert "nodejs" in corepack.parent.action.cmd


def test_chain_node_yarn_classic() -> None:
    deps = js.project(pm="yarn-classic").install()
    assert "yarn install --frozen-lockfile" in deps.action.cmd
    assert "corepack enable" in deps.parent.action.cmd
    assert "nodejs" in deps.parent.parent.action.cmd


def test_chain_node_yarn_berry() -> None:
    deps = js.project(pm="yarn-berry").install()
    assert "yarn install --immutable" in deps.action.cmd
    assert "corepack enable" in deps.parent.action.cmd
...
def test_chain_bun_runtime() -> None:
    ...
    assert "bun install --frozen-lockfile" in bun_deps.action.cmd

    bun_setup = bun_deps.parent
    assert "bun.sh/install" in bun_setup.action.cmd
    assert bun_setup.cache is not None

    apt_base = bun_setup.parent
    assert "apt-get" in apt_base.action.cmd
    assert "unzip" in apt_base.action.cmd
    assert apt_base.parent.action is None
...
def test_chain_node_bun_as_pm() -> None:
    ...
    assert "bun.sh/install" in bun_pm.action.cmd
...
def test_chain_deno() -> None:
    ...
    assert "deno install" in deno_deps.action.cmd

    deno_setup = deno_deps.parent
    assert "deno.land/install.sh" in deno_setup.action.cmd
...
def test_run_uses_npm_run() -> None:
    assert "npm run typecheck" in js.project().run("typecheck").action.cmd


def test_run_uses_pnpm_run() -> None:
    assert "pnpm run typecheck" in js.project(pm="pnpm").run("typecheck").action.cmd


@pytest.mark.parametrize("pm", ["yarn-classic", "yarn-berry"])
def test_run_uses_yarn_run(pm: str) -> None:
    assert "yarn run test" in js.project(pm=pm).run("test").action.cmd


def test_run_uses_bun_run() -> None:
    assert "bun run typecheck" in js.project(runtime="bun").run("typecheck").action.cmd


def test_run_uses_deno_task() -> None:
    assert "deno task typecheck" in js.project(runtime="deno").run("typecheck").action.cmd
...
def test_actions_respect_path() -> None:
    assert "cd packages/ui" in js.project(path="packages/ui").run("test").action.cmd
...
class TestAutoDetection:
    def test_detects_pnpm(self, tmp_path) -> None:
        ...
        assert "pnpm install --frozen-lockfile" in p.install().action.cmd

    def test_detects_bun(self, tmp_path) -> None:
        (tmp_path / "package.json").write_text("{}")
        (tmp_path / "bun.lock").touch()
        p = js.project(path=str(tmp_path))
        assert "bun install --frozen-lockfile" in p.install().action.cmd
        assert "bun.sh/install" in p.install().parent.action.cmd
...
class TestCorepackPin:
    def test_pnpm_command_includes_version(self, tmp_path) -> None:
        ...
        assert corepack.action.cmd == "corepack enable pnpm && corepack install -g pnpm@10.33.0"
    ...
    def test_yarn_berry_command_includes_version(self, tmp_path) -> None:
        ...
        deps = js.project(path=str(tmp_path)).install()
        corepack = deps.parent
        assert corepack.action.cmd == "corepack enable yarn && corepack install -g yarn@4.5.0"
    ...
    def test_command_has_no_version_when_field_absent(self, tmp_path) -> None:
        ...
        deps = js.project(path=str(tmp_path)).install()
        corepack = deps.parent
        assert corepack.action.cmd == "corepack enable pnpm"
