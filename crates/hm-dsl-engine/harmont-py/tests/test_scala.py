...
class TestScalaToolchain:
   ...
   def test_fmt_all_check_default(self):
      tc = hm.scala.toolchain(path=".")
      tc.fmt().cmd.endswith("sbt scalafmtAll")
...
class TestScalaProject:
   ...
   def test_warmup_implicit_cache_on_change(self):
      ...
      assert proj.warmup.cache == CacheOnChange(
         paths=('cli/build.sbt', 'cli/**/src/main/*.scala', 'cli/**/src/test/*.scala', 'cli/project/build.properties')
      )

   def test_warmup_implicit_cache_dot_path(self):
      proj = hm.scala.project(path=".")
      assert proj.warmup.cache == CacheOnChange(paths=('build.sbt', '**/src/main/*.scala', '**/src/test/*.scala', 'project/build.properties'))
   ...
   def test_fmt_command(self):
      proj = hm.scala.project(path="cli")
      assert proj.fmt().cmd.endswith("sbt scalafmtAll")
   ...
   def test_pipeline_ir(self):
      ...
      assert any("sbt scalafmtAll" in c for c in cmds)
