...
def test_golden_commands():
    ...
    assert _tail(p.test(features=("a", "b"), nextest=True).action.cmd) == (
        "cargo nextest run --workspace --features a,b --locked"
    )
    assert _tail(p.clippy(all_features=True).action.cmd) == (
        "cargo clippy --workspace --all-targets --all-features --locked -- -D warnings"
    )
    assert _tail(p.fmt().action.cmd) == "cargo fmt --all --check"
    assert _tail(p.doc(document_private_items=True).action.cmd) == (
        "cargo doc --no-deps --document-private-items --workspace --locked"
    )
    assert _tail(p.build(packages=("core",), target="wasm32-unknown-unknown").action.cmd) == (
        "rustup target add wasm32-unknown-unknown && "
        "cargo build -p core --target wasm32-unknown-unknown --locked"
    )
    assert (
        _tail(p.feature_powerset(subcommand="check", skip=("a b", "c")).action.cmd)
