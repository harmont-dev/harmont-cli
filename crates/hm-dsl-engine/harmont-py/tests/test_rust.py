...
class TestRustToolchain:
    ...
    def test_build_release(self):
        ...
        assert "cargo build --release" in s.action.cmd

    def test_test_release(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.test(release=True)
        assert "cargo test --release" in s.action.cmd
    ...
    def test_default_components(self):
        ...
        p = hm.pipeline([tc.build()])
        rustup = _step_by_substring(p, "sh.rustup.rs")
        assert "--component clippy,rustfmt" in rustup["action"]["cmd"]

    def test_components_override(self):
        tc = hm.rust.toolchain(path=".", components=("clippy",))
        p = hm.pipeline([tc.build()])
        rustup = _step_by_substring(p, "sh.rustup.rs")
        assert "--component clippy" in rustup["action"]["cmd"]
        assert "rustfmt" not in rustup["action"]["cmd"]

    def test_version_in_rustup_cmd(self):
        tc = hm.rust.toolchain(path=".", version="1.81.0")
        p = hm.pipeline([tc.build()])
        rustup = _step_by_substring(p, "sh.rustup.rs")
        assert "--default-toolchain 1.81.0" in rustup["action"]["cmd"]
    ...
    def test_warmup_returns_step(self):
        tc = hm.rust.toolchain(path="cli")
        w = tc.warmup()
        assert w.action.cmd is not None
        assert "cargo build --workspace --tests --locked" in w.action.cmd
    ...
    def test_build_locked_by_default(self):
        tc = hm.rust.toolchain(path=".")
        assert tc.build().action.cmd.endswith("cargo build --locked")

    def test_build_unlocked(self):
        tc = hm.rust.toolchain(path=".")
        assert tc.build(locked=False).action.cmd.endswith("cargo build")

    def test_build_features(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.build(features=("a", "b"), release=True)
        assert "cargo build --features a,b --release --locked" in s.action.cmd

    def test_build_packages(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.build(packages=("core", "cli"))
        assert "cargo build -p core -p cli --locked" in s.action.cmd

    def test_test_all_features(self):
        tc = hm.rust.toolchain(path=".")
        assert "cargo test --all-features --locked" in tc.test(all_features=True).action.cmd

    def test_test_nextest_switches_runner(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.test(nextest=True, workspace=True)
        assert "cargo nextest run --workspace --locked" in s.action.cmd

    def test_doctest_appends_doc(self):
        tc = hm.rust.toolchain(path=".")
        assert tc.doctest(workspace=True).action.cmd.endswith(
    ...
    def test_clippy_all_targets_locked_deny(self):
        tc = hm.rust.toolchain(path=".")
        assert "cargo clippy --all-targets --locked -- -D warnings" in tc.clippy().action.cmd

    def test_clippy_extra_lints(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.clippy(extra_lints=("-W clippy::pedantic",))
        assert "-- -D warnings -W clippy::pedantic" in s.action.cmd

    def test_clippy_no_deny(self):
        tc = hm.rust.toolchain(path=".")
        assert " -- " not in tc.clippy(deny_warnings=False).action.cmd

    def test_fmt_all_check_default(self):
        tc = hm.rust.toolchain(path=".")
        assert tc.fmt().action.cmd.endswith("cargo fmt --all --check")

    def test_doc_sets_rustdocflags_env(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.doc()
        assert "cargo doc --no-deps --locked" in s.action.cmd
        assert s.action.env == {"RUSTDOCFLAGS": "-D warnings"}

    def test_doc_respects_user_rustdocflags(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.doc(env={"RUSTDOCFLAGS": "-D rustdoc::all"})
        assert s.action.env == {"RUSTDOCFLAGS": "-D rustdoc::all"}

    def test_doc_no_deny(self):
        tc = hm.rust.toolchain(path=".")
        assert tc.doc(deny_warnings=False).action.env is None

    def test_test_all_targets(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.test(all_targets=True, workspace=True)
        assert "cargo test --workspace --all-targets --locked" in s.action.cmd

    def test_doctest_target(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.doctest(target="wasm32-unknown-unknown")
        assert s.action.cmd.endswith("cargo test --target wasm32-unknown-unknown --locked --doc")
        assert "rustup target add wasm32-unknown-unknown && cargo test" in s.action.cmd

    def test_test_nextest_target_auto_installs(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.test(nextest=True, target="wasm32-unknown-unknown")
        assert "rustup target add wasm32-unknown-unknown && cargo nextest run" in s.action.cmd

    def test_clippy_extra_lints_without_deny(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.clippy(deny_warnings=False, extra_lints=("-W clippy::pedantic",))
        assert s.action.cmd.rstrip().endswith("-- -W clippy::pedantic")
        assert "-D warnings" not in s.action.cmd

    def test_feature_powerset_default(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.feature_powerset()
        assert "cargo hack check --feature-powerset --depth 2 --no-dev-deps" in s.action.cmd
    ...
    def test_feature_powerset_each_feature(self):
        ...
        assert "--each-feature" in s.action.cmd
        assert "--feature-powerset" not in s.action.cmd
    ...
    def test_feature_powerset_skip_and_keep_going(self):
        ...
        assert expected in s.action.cmd
    ...
    def test_doc_target_auto_installs(self):
        ...
        assert "rustup target add wasm32-unknown-unknown && cargo doc" in s.action.cmd

    def test_no_target_no_rustup_add(self):
        tc = hm.rust.toolchain(path=".")
        assert "rustup target add" not in tc.build().action.cmd

    def test_target_value_quoted_in_rustup_add(self):
        tc = hm.rust.toolchain(path=".")
        s = tc.build(target="x; rm -rf /")
        assert "rustup target add 'x; rm -rf /' &&" in s.action.cmd
...
class TestRustProject:
    def test_project_has_all_methods(self):
        ...
        assert proj.warmup.action.cmd is not None
        assert proj.test().action.cmd is not None
        assert proj.clippy().action.cmd is not None
        assert proj.fmt().action.cmd is not None
    ...
    def test_test_command(self):
        proj = hm.rust.project(path="cli")
        assert "cargo test --workspace --locked" in proj.test().action.cmd

    def test_test_flags(self):
        proj = hm.rust.project(path=".")
        step = proj.test(flags=("--lib", "--no-fail-fast"))
        assert "cargo test --workspace --locked --lib --no-fail-fast" in step.action.cmd

    def test_clippy_command(self):
        proj = hm.rust.project(path="cli")
        assert (
            "cargo clippy --workspace --all-targets --locked -- -D warnings"
            in proj.clippy().action.cmd
        )
    ...
    def test_fmt_command(self):
        proj = hm.rust.project(path="cli")
        assert proj.fmt().action.cmd.endswith("cargo fmt --all --check")

    def test_fmt_flags(self):
        proj = hm.rust.project(path=".")
        assert (
            "cargo fmt --all --check --config-path x"
            in proj.fmt(flags=("--config-path", "x")).action.cmd
        )
    ...
    def test_test_packages(self):
        ...
        assert "cargo test -p core --locked" in step.action.cmd

    def test_test_nextest(self):
        proj = hm.rust.project(path=".")
        assert "cargo nextest run --workspace --locked" in proj.test(nextest=True).action.cmd

    def test_build_method_exists(self):
        proj = hm.rust.project(path=".")
        assert "cargo build --workspace --locked" in proj.build().action.cmd
        assert proj.build().parent is proj.warmup

    def test_doctest_method(self):
        proj = hm.rust.project(path=".")
        assert proj.doctest().action.cmd.endswith("cargo test --workspace --locked --doc")
        assert proj.doctest().label == ":rust: doctest"

    def test_clippy_packages(self):
        proj = hm.rust.project(path=".")
        step = proj.clippy(packages=("core",))
        assert "cargo clippy -p core --all-targets --locked -- -D warnings" in step.action.cmd

    def test_doc_method(self):
        proj = hm.rust.project(path=".")
        s = proj.doc()
        assert "cargo doc --no-deps --workspace --locked" in s.action.cmd
        assert s.action.env == {"RUSTDOCFLAGS": "-D warnings"}
    ...
    def test_ci_nextest_adds_doctest(self):
        ...
        assert any("cargo nextest run" in s.action.cmd for s in steps)
        assert any(s.action.cmd.endswith("--doc") for s in steps)
    ...
    def test_doc_exclude(self):
        ...
        assert "cargo doc --no-deps --workspace --exclude integration --locked" in s.action.cmd
    ...
    def test_feature_powerset_delegates(self):
        ...
        assert "cargo hack clippy --feature-powerset --depth 2 --no-dev-deps" in s.action.cmd
...
def test_no_shell_injection_via_packages():
    ...
    assert "-p 'a; touch /tmp/pwned'" in s.action.cmd
    assert "; touch /tmp/pwned --locked" not in s.action.cmd


def test_no_shell_injection_via_target():
    tc = hm.rust.toolchain(path=".")
    s = tc.build(target="x; rm -rf /")
    assert "--target 'x; rm -rf /'" in s.action.cmd
