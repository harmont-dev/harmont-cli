...
def _step_cmds(definition):
    return [n["step"].get("action", {}).get("cmd") for n in _graph_nodes(definition)]
...
def test_single_pipeline_no_triggers():
    ...
    nodes = _graph_nodes(definition)
    assert len(nodes) == 1
    assert nodes[0]["step"]["action"]["cmd"] == "echo hi"
...
def test_envelope_composes_targets_with_dedup(tmp_path, monkeypatch):
    ...
    definition = out["pipelines"][0]["definition"]
    nodes = _graph_nodes(definition)
    apt_nodes = [n for n in nodes if n["step"].get("action", {}).get("cmd") == "apt-get update"]
    ...
    child_cmds = sorted(n["step"]["action"]["cmd"] for n in children)
